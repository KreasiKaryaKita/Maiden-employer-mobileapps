...
class CommonFunction {
  static bool get isLoggedIn {
    var _pref = PreferenceHelper().getUser;
    if (_pref != null && _pref.id != null) {
      return true;
    } else {
      return false;
    }
  ...
  static bool isCreatedByMe(int id) {
    if (isLoggedIn) {
      var _pref = PreferenceHelper().getUser;
      if (id == _pref!.id) {
        return true;
  ...
  static String currency(num? text) {
    final formatCurrency = NumberFormat.simpleCurrency(locale: 'id_ID', name: 'Rp ', decimalDigits: 0);
    return text != null ? formatCurrency.format(text) : 'Rp. 0';
  }
  ...
  static copyToClipBoard({required String textToCopy, required String hint}) async {
    await FlutterClipboard.copy(textToCopy)
        .then((value) => snackbarHelper(message: '$hint berhasil di salin ke clipboard'));
  }
